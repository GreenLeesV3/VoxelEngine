...
- **`FluidSim::tick(&mut self, world: &mut World) -> usize`**: processes the active set once (see algorithm below), processing *every* active cell each tick so flow speed is volume-independent. No per-tick cell budget — the frame-level death-spiral guard (`MAX_STEPS_PER_FRAME` in `vox-platform`) handles the case where a tick is too slow to run in real time by running fewer ticks per frame (dilating simulated time uniformly), never by skipping cells within a tick.
