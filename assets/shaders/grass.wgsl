...
fn vs_main(in: VIn) -> VOut {
    ...
    if (in.height_factor > 0.5) {
        ...
        let wind_x = sin(t * 1.5 + world_pos.x * 0.7 + world_pos.z * 0.5) * 0.15 * cam.fog.z * in.height_factor;
        let wind_z = cos(t * 1.1 + world_pos.z * 0.9 + world_pos.x * 0.3) * 0.10 * cam.fog.z * in.height_factor;
        world_pos.x += wind_x;
        world_pos.z += wind_z;
    }
